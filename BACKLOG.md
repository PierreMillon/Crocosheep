...
- `[x]` **Heure affichée en retard d'1 seconde** : le message utilisait `serverTimestamp()` de Firestore à la fois pour le tri et pour l'heure affichée — cette valeur n'est figée qu'à la confirmation d'écriture (latence réseau réelle), pas au moment du clic, d'où le décalage. Ajouté un champ `clientTs` (horloge calée serveur, la même que l'algorithme caché utilise déjà — donc pas de perte de fiabilité anti-triche) capturé au moment exact du clic, affiché en priorité ; `ts` (Firestore) reste inchangé pour le tri et la déduplication, donc aucun risque sur les conversations déjà en base. Vérifié par test automatisé avec latence réseau simulée.
