...
- `[?]` **Vraies notifications app fermée — décision revue le 12/09, contredit l'ancienne ligne ci-dessous.** Pierre confirme que l'absence de notification quand l'appli est fermée est le vrai frein à l'usage quotidien (surtout avec l'objectif de faire grandir l'appli, voir plus bas). Pour de vraies notifications poussées par le serveur (quelqu'un t'envoie un mouton pendant que l'appli est fermée), il faut une Cloud Function déclenchée sur l'écriture Firestore — **ça exige le plan Blaze** (le plan gratuit Spark ne permet pas les Cloud Functions ni les appels sortants). Le plan Blaze a un quota gratuit généreux (l'usage actuel resterait à 0 €), mais il faut quand même attacher une carte bancaire au projet Firebase comme filet de sécurité — c'est une vraie action, pas juste une case à cocher, donc j'attends la confirmation explicite de Pierre avant de m'y lancer.
