...
- `[?]` **Pseudo local sur un contact** — Pierre demande mon avis sur pouvoir renommer un contact pour le reconnaître plus facilement. Avis donné : bonne idée, à faire en purement local/privé (jamais envoyé à Firestore, jamais vu par l'autre personne) — remplace l'affichage du code partout où c'est pertinent, code toujours consultable en dessous. Pas codé, en attente de son feu vert sur cette approche (ou d'une autre direction, ex. pseudo visible par l'autre = choix bien plus engageant côté confidentialité).
